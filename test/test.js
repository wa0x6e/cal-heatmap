...
QUnit.test("itemSelector accept a valid Element object", function (assert) {
	...
	assert.equal(
		...
		true,
		'd3.select("[data=y]")[0][0] is a valid itemSelector'
	);
	assert.equal(
...
QUnit.module("Unit Test: getDatas()", {
	beforeEach: function (assert) {
		path =
			(window.parent.document.title === "Karma" ? "base/test/" : "") +
			"data/";
	},
	...
	function (assert) {
		...
		assert.ok(
			cal.getDatas(
				...
				new Date(),
				function () {
					assert.ok(true, true, "Callback argument is called");
				},
				function (data) {
					assert.deepEqual(
						data,
						{},
						"undefined is equivalent to an empty object"
					);
				}
			)
		);
			...
			cal.getDatas(
				...
				function (data) {
					assert.deepEqual(
						data,
						{},
						"null is equivalent to an empty object"
					);
				}
			)
		);
			...
			cal.getDatas(
				...
				function (data) {
					assert.deepEqual(
						data,
						{},
						"number is equivalent to an empty object"
					);
				}
			)
		);
		...
		cal.getDatas(
			...
			function (data) {
				assert.deepEqual(
					data,
					{},
					"empty string is equivalent to an empty object"
				);
			}
		)
	);
		...
		cal.getDatas(
			...
			function (data) {
				assert.deepEqual(
					data,
					dt,
					"The passed object is used directly"
				);
			}
		),
		false
...
QUnit.test(
	"Passing a non-empty string is interpreted as an URL, and parsed using JSON",
	function (assert) {
		var ready = assert.async();
		assert.expect(3);
			...
			cal.getDatas(
				...
				function (data) {
					...
					assert.deepEqual(
						data,
						fileContent,
						"The file is read, and converted to a json object"
					);
				}
			),
			false
		...
		cal.getDatas(
			...
			function (data) {
				...
				assert.equal(
					data,
					fileContent,
					"The file is read as a plain text file"
				);
			}
		),
		false
...
QUnit.test(
	...
	function (assert) {
		...
		assert.strictEqual(
			cal.getHighlightClassName(new Date()),
			" highlight-now"
		);
	}
);
...
QUnit.test(
	...
	function (assert) {
		...
		assert.equal(
			domain[0].getHours(),
			"0",
			"Domain start hour is equal to 0"
		);
		assert.equal(
			domain[0].getMinutes(),
			"0",
	...
	cal.svg()
		...
		.each(function (domainStartDate) {
			...
			assert.equal(
				subDomain.length,
				24,
				"The day subdomain contains 24 hours"
			);
	...
	cal.svg()
		...
		.each(function (domainStartDate) {
			...
			assert.equal(
				subDomain[0].t,
				startDate.getTime(),
				"The week subdomain start is the first day of week : " +
					subDomain[0]
			);
			assert.equal(
				subDomain[subDomain.length - 1].t,
	...
	cal.svg()
		...
		.each(function (domainStartDate) {
			...
			assert.equal(
				subDomain[0].t,
				startDate.getTime(),
				"The week subdomain start is the first hour of week : " +
					subDomain[0]
			);
			assert.equal(
				subDomain[subDomain.length - 1].t,
	...
	cal.svg()
		...
		.each(function (domainStartDate) {
			...
			assert.equal(
				subDomain[0].t,
				startDate.getTime(),
				"The month subdomain start is the first day of month : " +
					subDomain[0]
			);
			assert.equal(
				subDomain[subDomain.length - 1].t,
	...
	cal.svg()
		...
		.each(function (domainStartDate) {
			...
			assert.equal(
				subDomain[0].t,
				startDate.getTime(),
				"The month subdomain start is the first hour of month : " +
					subDomain[0]
			);
			assert.equal(
				subDomain[subDomain.length - 1].t,
	...
	cal.svg()
		...
		.each(function (domainStartDate) {
			...
			assert.equal(
				subDomain[0].t,
				domainStartDate.getTime(),
				"The year subdomain start is the first month of year : " +
					subDomain[0]
			);
			assert.equal(
				subDomain[subDomain.length - 1].t,
	...
	cal.svg()
		...
		.each(function (d) {
			...
			assert.equal(
				subDomain[0].t,
				domainStartDate.getTime(),
				"The year subdomain start is the first week of year : " +
					subDomain[0].t
			);
...
QUnit.test(
	...
	function (assert) {
		...
		assert.equal(
			typeof d3
				.select("#previous")
				.on("click." + cal.options.itemNamespace),
			"function",
...
QUnit.test(
	...
	function (assert) {
		...
		assert.equal(
			$("#cal-heatmap .graph .graph-subdomain-group title")[0].firstChild
				.data,
			"==January=="
		);
	}
